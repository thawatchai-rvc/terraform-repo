...
#########################
resource "vsphere_virtual_machine" "d2s-spider-rancher" {
  # General Information
...
resource "vsphere_virtual_machine" "d2s-spider-cicd-master" {
  # General Information
  count            = var.vm_cicd_master_count
  name             = "${var.base_vm_name}-${var.vm_cicd_master_name}-${count.index + 1}"
  resource_pool_id = data.vsphere_resource_pool.pool_dats_tf_1.id 

  num_cpus = var.vm_cicd_master_num_vcpus_per_socket
  memory   = var.vm_cicd_master_memory_size_mib
  guest_id = var.guest_id # ใช้ guest_id สำหรับ Ubuntu 64-bit

  disk {
    label            = "disk0"
    size             = var.vm_cicd_master_disk_size_gb
    eagerly_scrub    = false
    datastore_id     = data.vsphere_datastore.datastore_lun01.id # อย่าลืมไปแก้ที่ General Information
    thin_provisioned = true
  }
  ...
  vapp {
    properties = {
      "user-data" = base64encode(templatefile(var.cloud_init_file_path, {
        hostname   = "${var.base_vm_name}-${var.vm_cicd_master_name}-${count.index + 1}",
        ip_address = var.vm_cicd_master_static_ips[count.index],
        gateway    = var.vm_gateway_ip
      }))
    }
  }
...
resource "vsphere_virtual_machine" "d2s-spider-cicd-node" {
  # General Information
  count            = var.vm_cicd_node_count
  name             = "${var.base_vm_name}-${var.vm_cicd_node_name}-${count.index + 1}"
  resource_pool_id = data.vsphere_resource_pool.pool_dats_tf_1.id

  num_cpus = var.vm_cicd_node_num_vcpus_per_socket
  memory   = var.vm_cicd_node_memory_size_mib
  guest_id = var.guest_id # ใช้ guest_id สำหรับ Ubuntu 64-bit

  disk {
    label            = "disk0"
    size             = var.vm_cicd_node_disk_size_gb
    eagerly_scrub    = false
    datastore_id     = data.vsphere_datastore.datastore_lun01.id # อย่าลืมไปแก้ที่ General Information
    thin_provisioned = true
  }
  ...
  vapp {
    properties = {
      "user-data" = base64encode(templatefile(var.cloud_init_file_path, {
        hostname   = "${var.base_vm_name}-${var.vm_cicd_node_name}-${count.index + 1}",
        ip_address = var.vm_cicd_node_static_ips[count.index],
        gateway    = var.vm_gateway_ip
      }))
    }
  }
