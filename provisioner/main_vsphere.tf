...
resource "vsphere_virtual_machine" "d2s-spider-provisioner" {
  # General Information
  name             = "${var.base_vm_name}-${var.vm_provisioner_name}"
  resource_pool_id = data.vsphere_resource_pool.pool_dats_tf_1.id # ใช้ resource pool แทน cluster เนื่องจากไม่มี
  # datastore_id     = data.vsphere_datastore.datastore_lun01.id    # อย่าลืมไปแก้ที่ disk

  num_cpus = var.vm_provisioner_num_vcpus_per_socket
  memory   = var.vm_provisioner_memory_size_mib
  guest_id = var.guest_id # ใช้ guest_id สำหรับ Ubuntu 64-bit

  disk {
    label            = "disk0"
    size             = var.vm_provisioner_disk_size_gb
    eagerly_scrub    = false
  ...
  vapp {
    properties = {
      "user-data" = base64encode(templatefile(var.cloud_init_file_path, {
        hostname   = "${var.base_vm_name}-${var.vm_provisioner_hostname}",
        ip_address = var.vm_provisioner_ip,
